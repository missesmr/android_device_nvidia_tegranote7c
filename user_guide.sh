...
USER_GUIDE_EN_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_en_copy_done_v3
...
USER_GUIDE_DE_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_de_copy_done_v3
...
USER_GUIDE_ES_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_es_copy_done_v3
...
USER_GUIDE_FR_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_fr_copy_done_v3
...
USER_GUIDE_IT_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_it_copy_done_v3
...
USER_GUIDE_RU_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_ru_copy_done_v3

USER_GUIDE_PL_SRC_FILE=/system/media/TegraNOTE7LTEUserGuide-pl.pdf
USER_GUIDE_PL_DST_FILE=$USER_GUIDE_DST_DIR/TegraNOTE7LTEUserGuide-pl.pdf
USER_GUIDE_PL_COPY_DONE_FILE=$USER_GUIDE_DST_DIR/.user_guide_pl_copy_done_v3
...
if [ -d $EMULATED_STORAGE_SOURCE/0 ]; then
    ...
    if [ -f "$USER_GUIDE_PL_SRC_FILE" ]; then
        if [ ! -f "$USER_GUIDE_PL_COPY_DONE_FILE" ]; then
            mkdir -p $USER_GUIDE_DST_DIR
            cp $USER_GUIDE_PL_SRC_FILE $USER_GUIDE_PL_DST_FILE
            echo "user guide copy done" > $USER_GUIDE_PL_COPY_DONE_FILE
        fi
    fi

    if [ -f "$USER_GUIDE_JA_DST_FILE" ]; then
        rm -f $USER_GUIDE_JA_DST_FILE
    fi

    if [ -f "$USER_GUIDE_KO_DST_FILE" ]; then
        rm -f $USER_GUIDE_KO_DST_FILE
    fi

    if [ -f "$USER_GUIDE_ZH_DST_FILE" ]; then
        rm -f $USER_GUIDE_ZH_DST_FILE
    fi

    if [ -f "$USER_GUIDE_ZT_DST_FILE" ]; then
        rm -f $USER_GUIDE_ZT_DST_FILE
    fi

    if [ -f "$USER_GUIDE_PT_DST_FILE" ]; then
        rm -f $USER_GUIDE_PT_DST_FILE
    fi
fi
